...
// ════════════════════════════════════════════════════
// ════════════════════════════════════════════════════
// AUTO TRANSLATE POI FIELDS
// ════════════════════════════════════════════════════
async function autoTranslatePOI(sourceId, targetId, lang) {
  const sourceText = document.getElementById(sourceId).value;
  if (!sourceText) {
    showToast('Vui lòng nhập text tiếng Việt trước!', 'error');
    return;
  }
  ...
  try {
    const translated = await autoTranslate(sourceText, lang);
...
async function deletePOI(id) {
  ...
  if (!p) return;
  if (!confirm('Xóa điểm "' + p.name + '"? Hành động không thể hoàn tác!')) return;
  try {
...
async function uploadAudio() {
  ...
  const poiName = poisData.find(p => p.id === poiId)?.name || poiId;
  ...
  tbody.innerHTML = poisData.map(p => {
    ...
    <tr>
      ...
      <td style="cursor:${hasJa?'pointer':'default'}"
        ...
        ${hasJa
          ? '<span class="badge badge-active">✓ Có</span>'
          : '<span class="badge badge-inactive">— Chưa có</span>'}
      </td>
    </tr>
  `;
  }).join('');
...
function switchLangTab(el, group) {
  ...
  // Fallback cho group dạng "name-vi", "addr-en"...
...
// ════════════════════════════════════════════════════
// KHỞI ĐỘNG — Auth gate: chỉ load data khi đã đăng nhập
// ════════════════════════════════════════════════════
onAuthStateChanged(auth, user => {
  if (user) {
    // Đã đăng nhập: ẩn login screen, hiện app
    ...
    // Chưa đăng nhập: hiện login screen, ẩn app
...
async function autoTranslate(text, targetLang = 'en') {
  ...
  try {
    ...
    console.error('Dịch lỗi:', error);
  }
  return '';
}

// Hàm gắn auto-translate vào input
function setupAutoTranslate(inputElementId, targetInputId, language = 'en') {
  ...
  input.addEventListener('input', function() {
    ...
    debounceTimer = setTimeout(async () => {
      const translated = await autoTranslate(this.value, language);
...
// Expose translation function to global scope
window.autoTranslate = autoTranslate;
window.setupAutoTranslate = setupAutoTranslate;
...
function loadPOIForTranslate() {
  ...
  // Populate original Vietnamese fields
...
function getExistingTranslation(poiId, lang) {
  ...
  // Kiểm tra format mới (translations object)
  if (poi.translations && poi.translations[lang]) {
    return poi.translations[lang];
  }
  
  // Kiểm tra format cũ (Name_en, Name_ja, v.v.)
  if (lang === 'en') {
    return {
      Name: poi.Name_en || '',
...
async function saveTranslation() {
  ...
  try {
    ...
    updateData[`langs`] = arrayUnion(currentTranslateLang); // Thêm ngôn ngữ vào mảng langs
    
    await updateDoc(docRef, updateData);
    showToast(`Đã lưu bản dịch ${langLabels[currentTranslateLang]}!`, 'success');
    closeModal('modal-translate');
    renderTranslations(); // Cập nhật bảng
  } catch (e) {
    showToast('Lỗi lưu: ' + e.message, 'error');
  }
}
...
async function loadAccessLogs() {
  const PROJECT_ID = "vinh-khanh-cms";
  const API_KEY = "AIzaSyDO7cvTxvx26Qu6Bo6Ts5ZT0cl8yBhcj5s";

  const url = `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/(default)/documents/app_access_logs?key=${API_KEY}`;
  ...
  try {
    ...
    console.error("Load access logs lỗi:", err);
  }
}
...
function renderAccessLogs(data) {
  ...
  data.forEach(item => {
    const last = new Date(item.LastActive);
...
// ════════════════════════════════════════════════════
// VIEW & EDIT TRANSLATION FUNCTIONS
// ════════════════════════════════════════════════════
...
function viewTranslation(poiId, lang) {
  ...
  // Lấy từ format mới hoặc format cũ
...
// Expose functions to global scope (cần cho onclick trong HTML)
window.switchPanel    = switchPanel;
window.openAddPOI     = openAddPOI;
window.editPOI        = editPOI;
window.autoTranslatePOI = autoTranslatePOI;
...
window.loadPOIForTranslate    = loadPOIForTranslate;
window.autoTranslateField     = autoTranslateField;
